/*
 * YouCord — Local un-injector for Discord Desktop
...
function findAllDiscordResources() {
    ...
    // Filtrer uniquement les paths avec une injection YouCord présente
...
function uninject(resourcesDir) {
    ...
    // Vérifier que le dossier app/ a bien été créé par YouCord
    if (existsSync(appDirPath)) {
        try {
            if (existsSync(join(appDirPath, "index.js"))) {
                ...
                if (!indexContent.includes("YouCord Injector") && !indexContent.includes("YouCord")) {
                    console.warn(`\x1b[33m[YouCord] Le dossier app/ existe mais n'a pas l'air d'avoir été créé par YouCord.\x1b[0m`);
        ...
        console.log("[YouCord] Suppression du dossier app/ injecté...");
        rmSync(appDirPath, { recursive: true, force: true });
    } else {
        console.log("\x1b[33m[YouCord] Aucun dossier app/ injecté trouvé.\x1b[0m");
    }

    // Restaurer le backup
    if (existsSync(backupPath) && !existsSync(appAsarPath)) {
        console.log("[YouCord] Restauration _app.asar → app.asar...");
        renameSync(backupPath, appAsarPath);
    } else if (existsSync(backupPath) && existsSync(appAsarPath)) {
        console.log("[YouCord] app.asar déjà présent, nettoyage du backup...");
        rmSync(backupPath, { force: true });
    }

    console.log(`\x1b[32m[YouCord] Désinjection réussie depuis : ${resourcesDir}\x1b[0m`);
    console.log("\x1b[36m[YouCord] Redémarrez Discord pour appliquer les changements.\x1b[0m");
    return true;
}
...
if (allResources.length === 0) {
    console.error("\x1b[31m[YouCord] Aucune installation Discord avec YouCord injecté trouvée.\x1b[0m");
    console.error("\x1b[33m           Assurez-vous que YouCord a bien été injecté via 'pnpm inject'.\x1b[0m");
    process.exit(1);
}
...
if (uninjectCount === 0) {
    console.error("\x1b[31m[YouCord] Aucune désinjection réussie.\x1b[0m");
    process.exit(1);
}

console.log(`\n\x1b[32m[YouCord] ${uninjectCount}/${allResources.length} désinjection(s) réussie(s).\x1b[0m`);
